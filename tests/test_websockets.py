...
class TestWebSockets(object):
    def test_connection_refused(self):
        ...
        with client.websocket_connect('/ws/r1/2023/'):
            room = manager.room_connections.get('r1')
            ...
            with pytest.raises(WebSocketDisconnect):
                with client.websocket_connect('/ws/r1/2023/'):
                    assert len(room) == 1  # 连接被拒且仍为1个活跃连接.
    ...
    async def test_broadcast(self):
        ...
        with (client_a.websocket_connect('/ws/r0/1001/') as websocket_a,
              client_b.websocket_connect('/ws/r0/1002/') as websocket_b):
            # 模拟由客户端a(实际上由服务器)发起广播, 且对自身广播.
            await manager.broadcast(s_data_a, 'r0')
            # 由客户端a发起广播.
            ...
            room = manager.room_connections.get('r0')
